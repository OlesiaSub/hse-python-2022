...
class MyASTVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: _ast.FunctionDef) -> Any:
        self.ast_graph.add_node(str(node), label=f'function: {node.name}', color='#CD8C95', style='filled')
        for a in node.args.args:
            self.ast_graph.add_node(a, label=f'arg: {a.arg}', color='#8B3E2F', style='filled')
    ...
    def visit_Assign(self, node: _ast.Assign) -> Any:
        self.increase_cnt()
        self.ast_graph.add_node(str(node), label="assignment", color='#53868B', style='filled')
    ...
    def visit_Name(self, node: _ast.Name) -> Any:
        ret_list = [str(node)]
        self.ast_graph.add_node(str(node), label=node.id, color='#009ACD', style='filled')
    ...
    def visit_Expr(self, node: _ast.Expr) -> Any:
        self.ast_graph.add_node(str(node), label="expr", color='#00C957', style='filled')
        self.process_visit_result(node, node.value)
        return [str(node)]

    def visit_Call(self, node: _ast.Call) -> Any:
        self.ast_graph.add_node(str(node), label="call", color='#00C957', style='filled')
    ...
    def visit_For(self, node: _ast.For) -> Any:
        self.increase_cnt()
        self.ast_graph.add_node(str(node), label="for", color='#8B6914', style='filled')
    ...
    def visit_If(self, node: _ast.If) -> Any:
        self.ast_graph.add_node(str(node), label="if", color='#8B6914', style='filled')
    ...
    def visit_BinOp(self, node: _ast.BinOp) -> Any:
        self.ast_graph.add_node(str(node), label="BinOp", color='#8A2BE2', style='filled')
    ...
    def visit_Compare(self, node: _ast.Compare) -> Any:
        self.increase_cnt()
        self.ast_graph.add_node(str(node), label="comparison", color='#00C957', style='filled')
        self.process_visit_result(node, node.left)
    ...
    def visit_Load(self, node: _ast.Load) -> Any:
        self.ast_graph.add_node(str(node) + str(self.load_cnt), label="ctx=Load", color='#808080', style='filled')
        self.load = str(node) + str(self.load_cnt)
        return [self.load]

    def visit_Store(self, node: _ast.Store) -> Any:
        self.ast_graph.add_node(str(node) + str(self.store_cnt), label="ctx=Store", color='#808080', style='filled')
        self.store = str(node) + str(self.store_cnt)
        return [self.store]

    def visit_Add(self, node: _ast.Add) -> Any:
        self.ast_graph.add_node(str(node), label="op=Add", color='#8A2BE2', style='filled')
        return [str(node)]

    def visit_Eq(self, node: _ast.Eq) -> Any:
        self.ast_graph.add_node(str(node), label="cond=Eq", color='#8A2BE2', style='filled')
        return [str(node)]

    def visit_Constant(self, node: _ast.Constant) -> Any:
        self.ast_graph.add_node(str(node), label="const: " + str(node.value), color='#009ACD', style='filled')
        return [str(node)]
